...
def _process_thead(
    ...
    for value in raw_aligns:
        if ALIGN_CENTER.match(value):
            aligns.append("center")
        elif ALIGN_LEFT.match(value):
            aligns.append("left")
        elif ALIGN_RIGHT.match(value):
            aligns.append("right")
        else:
            aligns.append(None)
    children: List[Dict[str, Any]] = [
        {
            "type": "table_cell",
            "text": text.strip(),
            "attrs": {"align": aligns[index], "head": True},
        }
    ...
    children: List[Dict[str, Any]] = [
        {
            "type": "table_cell",
            "text": value.strip(),
            "attrs": {"align": aligns[index], "head": False},
        }
...
def table(md: "Markdown") -> None:
    """A Mistune plugin for pipe-delimited Markdown tables."""
